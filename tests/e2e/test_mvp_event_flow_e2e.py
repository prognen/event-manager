...
class TestMVPEventManagementFlow:
    ...
    2. Вход в систему
    3. Просмотр справочников (площадки, активности, размещения, программы)
    4. Создание мероприятия с программой участия через /session/new
    ...
    async def test_full_event_management_scenario(self) -> None:
        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                with allure.step("Шаг 6: Просмотр доступных программ"):
                    programs_resp = await client.get("/program.html")
                    assert programs_resp.status_code == 200

                with allure.step("Проверка наличия программ"):
                    assert "Автомобиль" in programs_resp.text, (
                        "Программы с транспортом 'Автомобиль' не найдены"
                    )

                # ---- ACT: создание мероприятия ----
                with allure.step(
                    "Шаг 7: Создание мероприятия с программой участия Москва → Воронеж"
                ):
                    session_payload = {
                        "start_venue": "1",
                        "end_venue": "2",
                        "transfer_type": "Автомобиль",
